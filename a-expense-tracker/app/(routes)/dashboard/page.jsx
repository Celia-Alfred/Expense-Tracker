...
import BudgetAlertClient from '@/app/_components/BudgetAlertClient';
    ...
    <div className="mt-16 md:mt-0 text-white p-8">
      {user ? (
        <div>
          ...
          {budgetList.length > 0 && <BudgetAlertClient budgets={budgetList} />}
        </div>
      ) : (
        <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 mt-10 gap-5">
        ...
        <div className="grid gap-3">
          ...
          {user
            ...
            : [1, 2, 3, 4].map((index) => (
                <div key={index} className="rounded-lg bg-slate-800 animate-pulse h-[150px]"></div>
              ))}
        </div>
      </div>
    </div>
